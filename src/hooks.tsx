...
export function useAnimatedDynamicRefs(): [ContextType['refMap'], ContextType['setRef']] {
  ...
  const setRef = useCallback(function <T extends RefComponent>(key: TabName, ref: React.RefObject<T>) {
...
export function useCollapsibleStyle(): CollapsibleStyle {
  ...
  const containerHeightWithMinHeader = Math.max(0, (containerHeightVal ?? 0) - minHeaderHeight);
...
export function useScroller<T extends RefComponent>() {
  ...
  const scroller = useCallback(
      (ref: Ref<T> | undefined, x: number, y: number, animated: boolean, _debugKey: string) => {
          'worklet';
          if (!ref) return;
